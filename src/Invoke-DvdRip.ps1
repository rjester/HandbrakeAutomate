    ...
    while (-not $proc.HasExited) {
        ...
        try {
            ...
            if ($out -match 'PRGV:(\d+),(\d+),(\d+)') {
                ...
                if ($new -ne $percent) { $percent = $new; Write-Progress -Activity 'MakeMKV Rip' -PercentComplete $percent }
            }
        } catch { }
    }
    ...
    while (-not $proc.HasExited) {
        ...
        try {
            ...
            if ($jsonOut -match '"Progress"\s*:\s*([0-9.]+)') {
                $p = [double]$Matches[1]
                Write-Progress -Activity "Encoding: $(Split-Path $InputFile -Leaf)" -PercentComplete ([math]::Round($p * 100,1))
            }
        } catch { }
    }
